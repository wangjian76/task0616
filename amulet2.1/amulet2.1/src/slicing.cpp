...
static int err_par_in_slice = 32; // error in upwards_slicing  //改变
/*------------------------------------------------------------------------*/
// Global var
std::vector<std::list<Gate*>> slices;//门矩阵放在list里，再都放在vector里
/*------------------------------------------------------------------------*/

void init_slices() {
  for (unsigned i = 0; i < NN; i++) {
    std::list<Gate*> l;
    Gate * n = gates[i+M-1]; //添加每个切片的输出门 gates[M-1] gates[M]...gates[M+NN]
    assert(n->get_output());
    l.push_back(n);
    slices.push_back(l);
  }
...
void print_slices() { //将切片中的多项式打印到stdout
  for (int i = NN-1; i >= 0; i--) {
    msg("");
    msg("Slice %i", i);
    msg("");
    std::list<Gate*> l = slices[i]; //容器声明
...
const Gate * topological_largest_child(const Gate * n) {
  int slice = n->children_front()->get_slice(); //n为切片成员的子代位置最高的元素
  for (std::list<Gate*>::const_iterator it_n = n->children_begin(); //用it_n迭代子代
      it_n != n->children_end(); ++it_n) {
    Gate * n_child = *it_n;
    if (n_child->get_slice() != slice) //子代的门不等于输出门？
      die(err_top_child, "error in topological_largest_child with %s", n->get_var_name());
  }  //拓扑最大子节点错误

  Gate * n1 = n->children_front();

  for (std::list<Gate*>::const_iterator it_s = slices[n1->get_slice()].begin(); //用it_s迭代
      it_s != slices[n1->get_slice()].end(); ++it_s) {
    Gate * n_slice = *it_s;
    for (std::list<Gate*>::const_iterator it_n = n->children_begin();
        it_n != n->children_end(); ++it_n) {
      Gate * n_child = *it_n;
      if (n_slice == n_child) return n_child; //目的在于返回最高位置子代
    }
  }
  return 0;
}
...
void fix_slice(Gate * n, int i) {
  ...
  if (n->get_elim()) return;  //预处理过程中被消除了则返回
  n->set_slice(i); //将门n移到切片i上
  for (std::list<Gate*>::const_iterator it = slices[i].begin();
      it != slices[i].end(); ++it) {
    Gate * n_it = *it;
    if (n_it == after) {  //n_it = 拓扑的最高位置的子代
      slices[i].insert(it, n);
      //如果it=拓扑的最高位置的子代，在链表的位置it处插入元素n，就是n插在最高子代前
      return;
    }
  }
  slices[i].push_back(n);  //在插入到slices的这个链表里
}
/*------------------------------------------------------------------------*/
