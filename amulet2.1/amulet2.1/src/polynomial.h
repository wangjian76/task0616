...
class Polynomial {
  ...
  Monomial ** mon; //改进   // /< sorted deque of monomials
 ...
 public:
  ...
  Polynomial (Monomial ** m, size_t len); //改进
  ...
  */
  ...
  Monomial * get_mon(size_t i) const {   //改进
     if(i < num_mon) return mon[i];
     else return 0;
   }

   size_t size() const { return num_mon;} //改进

  /** Returns the leading term

      @return Term*
  */
  Term * get_lt() const {return mon[0]->get_term();} //改进
